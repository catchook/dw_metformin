...
###########ms
    sql= """
    select distinct a.cohort_definition_id
      ,(case when cohort_definition_id = target then 'T'
      ...
      left join 
      cdm_hira_2017.drug_exposure as b
      on a.subject_id = b.person_id
      left join 
      cdm_hira_2017.measurement as c
      on a.subject_id = c.person_id
      where a.cohort_definition_id in (target, control)
      and b.drug_concept_id in (select distinct descendant_concept_id
                                from cdm_hira_2017.concept_ancestor
                                where ancestor_concept_id in (1503297,43009032,19122137,
35198118,1502855,1502809, 43009070,1580747,793143, 40166035,1547504,1516766,
1525215,35197921, 1502826, 43009094, 1510202,43009055, 44506754, 40170911, 40239216,
43009020, 1559684, 19097821, 1560171,1597756, 19059796, 19001409, 43009089, 1583722,
43009051, 793293, 45774751, 45774435, 44785829, 1594973,19033498, 43526465, 43008991,43013884,44816332, 1530014, 1529331)  and c.measurement_concept_id in (3015183, 3020460, 3010156, 3013707))       and b.drug_exposure_start_date between a.cohort_start_date + interval '90 day' and a.cohort_start_date + interval '1 year 3 month'
    """
