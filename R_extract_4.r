...
print(c(database, user, pw, url, dbhost, port, schema))
####only for 원광대 
## 기존 쿼리로는 계속 실행되지 않는 문제 발생
##1차 시도: 쿼리 쪼개기 시도 
## 1 : 약물만 (서브쿼리, IN -> JOIN으로 변경 )
## 2 : 검사 
## 3 : 인구학적 정보 
#############1차 결합 
## 4 : 고혈압 약제 
...
JOIN
    (select z.person_id as person_id, z.condition_start_date as condition_start_date, y.ancestor_concept_id as ancestor_concept_id 
        ...
        join (SELECT descendant_concept_id, ancestor_concept_id
              FROM cdm_hira_2017.concept_ancestor
              WHERE ancestor_concept_id IN (4329847,316139, 321052 ,381591,434056,4182210, 4063381, 257628, 134442, 80800, 80809, 256197,255348, 4247120 ,4064161, 4212540, 201820, 443767, 442793, 192606, 374022, 4030518,4239233, 4245042, 443392, 4245975, 4029488, 192680,24966, 432851, 439727, 316866, 432867, 132797, 4254542 ) ) as y
